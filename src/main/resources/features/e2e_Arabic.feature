Feature: Test Offline Audio file - AR
  ...
  Scenario Outline: Validate Entity,Disposition and Alert Creation and Training.
    ...
    #############################################
    #
    ##ARFINITION AND CONFIGURATION` FOR ENTITY AND INTENT
    #
    ...
    #ENTITY
    #
    Then define and configure entities in folder "ConfigAndDefine/AR/entities/"
    #Then validate and configure rules in folder "ConfigAndDefine/AR/ruleEntity"
    #
    #SUMMARY FORMAT
    #
    ...
    #INTENT
    #
    Then define and configure call categorization with folder "ConfigAndDefine/AR/call-categorization"
    #
    #
    Then configure alerts in folder "ConfigAndDefine/AR/alerts"
    #
    ###########################################
    #
    ...
    Examples: 
      ...
      | "APITesting" | "AR"     | "APITesting" | "APITesting@uniphore.com" | "Agent" | "E"      | "audio-files/ARAudioFile/AR.wav" | "description" | "colorSample" |
  ...
  Scenario Outline: Validate Audio offline processing and compare generated Transcripts with Golden Std.
    ...
    Examples: 
      ...
      | "audio-files/ARAudioFile/AR.wav" |    0 | "مرحبا معك سوسن خدم ه عملاء الشركه جوتشي لل كيف ممكن اقدر اخدمك اليوم" | "src/test/resources/transcript-jsons/AR.json" |

  @AR @smoke @Regression
  Scenario Outline: Validate Summary.
    And a summary for callId exists
    And a summary for callId has intent of <intent>
    And a summary for callId has "اسم العميل" <اسم العميل>
    And a summary for callId has "جيوب في الحقيبة" <جيوب في الحقيبة>
    And a summary for callId has "حقيبة محمولة" <حقيبة محمولة>
    And a summary for callId has "معلم معروف" <معلم معروف>

    Examples: 
      | intent                   | اسم العميل | جيوب في الحقيبة  | حقيبة محمولة  | معلم معروف | 
      | "استعلم عن قسيمه/امتداد الصلاحية/مرفوض" | "سوسن"     | "3" | " شنطه المحموله" | "احمد و عاده"       |    

  @AR @smoke @Regression
  Scenario Outline: Validate Feedback loop for Summary and Disposition.
    Then edit "اسم العميل" as "محمد"
    Then submit the edited summaries
    Then compare if "اسم العميل" has "محمد" for callId
    And disposition for callId has intent of <intent>

    Examples: 
      | intent                                  |
      | "استعلم عن قسيمه/امتداد الصلاحية/مرفوض" |

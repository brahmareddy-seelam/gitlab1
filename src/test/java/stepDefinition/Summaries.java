...
public class Summaries extends BaseClass {
	...
	public void a_summary_for_callid_exists_with_intent() throws IOException, URISyntaxException, JSONException {
		...
		request.log().all().header("Content-Type", "application/json").header("Authorization",
				"Bearer " + TestCenter.accesstoken);
		...
		for (String str : entitiesStr.toString().substring(1, entitiesStr.length() - 1).split(",\"")) {
			...
			String entityVal = (indexOfSeparation + 2 < str.length()
					? str.substring(indexOfSeparation + 2, str.length() - 1)
					: str.substring(indexOfSeparation + 2, str.length() - 0));
	...
	public void a_summary_for_callid_has_entity_in_summary(String entityName, String entityValue)
	...
	public void post_summary_format() throws IOException, URISyntaxException, JSONException {
		...
		request.log().all().auth().oauth2(token).header("Content-Type", "application/json")
				.header("Authorization", "Bearer " + TestCenter.accesstoken).queryParams(CommonSteps.commonMap);
...
//	
	...
	public void edit_and_submit_summary(String key, String value) {
		...
		if(summariesMap != null) {
			...
			summariesMap=new LinkedHashMap<>();
			summariesMap.put(key, value);
		}
	}
	
	public void editAndSubmit(String key, String value) {
	...
	public void submit_edited_summaries() {
		...
		String summary = "[{\"requestId\":\"" + requestId + "\",\"data\":{\"format\":\"bulletPoint\",\"bullet\": "
				+ bulletValue + ",\"paragraph\":null}}]";
		loadURL("BACKEND_PORT");
	...
	public String getUserId() {
		...
		response = request.log().all().formParams(map)
				.post("auth/realms/" + port.getProperty("realm") + "/protocol/openid-connect/token");
...
//		JSONObject dateParams = new JSONObject();
		...
		request.log().all().header("Authorization", "Bearer " + TestCenter.accesstoken)
				.header("Content-Type", "application/json").queryParams(date);
